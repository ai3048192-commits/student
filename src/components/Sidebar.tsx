...
const menuItems: MenuItem[] = [
  ...
  { name: "الاشتراكات والدفع (vodafone/insta)", icon: CreditCard, path: "/subscriptions" },
  { name: "الإشعارات", icon: Bell, path: "/notifications", showCount: true },
...
export default function Sidebar({ isOpen, onClose, onSignOut }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        aria-label="القائمة الجانبية للطالب"
          ...
          <div className="space-y-6 overflow-y-auto pr-1">
            ...
            <nav className="space-y-1.5">
              {menuItems.map((item) => {
                ...
                const badge =
                  item.badge ?? (item.showCount && notificationsCount > 0 ? String(notificationsCount) : null);
                ...
                return (
                  ...
                  >
                    ...
                    {badge && (
                      <span
                        className={`relative z-10 shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${
                          isActive
                            ? "bg-white text-blue-600"
                            : badge === "مباشر"
                              ? "bg-rose-100 text-rose-600"
                              : "bg-blue-100 text-blue-600 tabular-nums"
                        }`}
                      >
                        {badge}
                      </span>
